...
**Java**：语法繁杂更新缓慢算50分，但开源的理念好，用的人多，不好的就重新封装一个，反正都能看到源码，改就是了，最终一些大公司就出品了一堆优秀的组件，当你想造一个轮子的时候发现已经有了，忍不住想干一票大的在JVM上造个语言替代java时，发现也有了Kotlin、Groovy、Scala。然后最终表现就是：50+30+10+15+........很容易超过了C#。一言以蔽之：**众星璀璨**、**百花齐放**。

Java的方式也有缺陷：自身不足第三方补足，然后Java后续版本又加上了第三方的内容可能类名都相同，就冲突了。Java后来发展好了第三方的组件可能就鸡肋了，或者一堆公司实现了同样的功能从而乱花渐欲迷人眼了。

java的语法当然可以做到像C#一样优秀，如：Kotlin,Grovvy就已经做到了，但为什么没人用呢？因为他不是java、不是官方的！只有当这些语法被纳入JDK才会有大量的人用，而JDK在做的事貌似就是缓慢的吸收这些语言的优点，一点一点加入JDK，慢慢等了您！就是这么现实。可以看到java语法的未来，但依然要忍受着当下。在经历过C#等更高级语法的人眼里：java的每一个迭代都是那么的理所当然、毫无波澜、没有惊喜。
...
Name|URL|Desc
----|----|----
Apache Commons Lang|https://commons.apache.org/proper/commons-lang/<br>https://github.com/apache/commons-lang|Commons的开发者会尽其所能地减少组件与其它开发库的依赖，让部署这些组件更加容易。除此之外，Commons组件还会尽可能保持接口的稳定，让Apache的用户（包括使用Commons的其它Apache项目）可以使用时无需担心未来可能的变化。内容包括：二进制，各种编码，字符串操作，集合扩展与增强，压缩解压，文件操作等。**最最常用的还是commons-lang3，一定要学会并优先用这种，节约时间减少bug的利器，比JDK写法简单！**
