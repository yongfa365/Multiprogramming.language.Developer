...
class HttpLoggingInterceptor @JvmOverloads constructor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        logger.log("")
        logger.log(
                "<-- ${response.code}${if (response.message.isEmpty()) "" else ' ' + response.message} ${response.request.url} ${response.protocol} (${tookMs}ms${if (!logHeaders) ", $bodySize body" else ""})")
