...
//fixedRate      到点就执行，如果上一个没执行完就等着，上个完成后他就立即执行。
...
public class ScheduleInSpring {
    ...
    @Scheduled(cron = "*/2 * * * * *")
    public static void taskByCron() throws Exception {
    ...
    public static void taskByFixedDeplay() throws Exception {
    ...
    public static void taskByFixedRate() throws Exception {
    ...
    @Scheduled(fixedRate = 3000)
    public static void taskByFixedRate2() throws Exception {
    ...
    //与jdk里的行为一致Executors.newSingleThreadScheduledExecutor().scheduleAtFixedRate
    //如果任务第一次要5s，之后只要0s，1s调度1次，则：第一次5s，然后紧接着会再执行4次之前落下的，然后才1s1次
    @Scheduled(fixedRate = 1000)
    public static void taskByFixedRate3() throws Exception {
